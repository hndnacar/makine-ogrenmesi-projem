...
from Gürültülü_python import Ui_Form4
...
class GurultuluPage(QWidget):
    def __init__(self) -> None:
        super().__init__()
        self.gurultuluform = Ui_Form4()
        self.gurultuluform.setupUi(self)# ana metodu self ile çağırıcam.
        ...
        self.gurultuluform.pushButton_veriSetiYkle.clicked.connect(self.veri_seti_yukle)

        # "Veri Setini Göster" butonunu tıklama olayına bağla
        self.gurultuluform.pushButton_veriSetiGster.clicked.connect(self.veri_setini_goster)

        # "Sayısal Olmayan Sütunları Kategorize Et" butonunu tıklama olayına bağla
        self.gurultuluform.pushButton_onslemeHazrlk.clicked.connect(self.kategorize_et)

        self.gurultuluform.pushButton_EksikVerileriDoldur.clicked.connect(self.eksik_veri_doldur)

        self.gurultuluform.pushButton_veriSetiniEgit.clicked.connect(self.train_model)

        self.gurultuluform.pushButton_predict_proba.clicked.connect(self.predict_methods)

        self.gurultuluform.pushButtonMetrikler.clicked.connect(self.show_metrics)

        # "Model Eğitimini Göster" butonunu tıklama olayına bağla
        self.gurultuluform.pushButtonModelOrnek.clicked.connect(self.open_egitim_sonucu)
        ...
        try:
            ...
            for row_index, row in veri_goruntu.iterrows():
                for col_index, value in enumerate(row):
                    # Hücredeki değeri işlemek
                    if pd.isna(value):  # Eğer değer eksikse
                        item = QTableWidgetItem("NaN")  # Eksik veri için özel metin
                    else:
                        item = QTableWidgetItem(f"{value}")  # Tüm değerleri metin olarak ekle
    ...
    def eksik_veri_doldur(self):
        if self.veri_seti is None:
            QMessageBox.warning(self, "Uyarı", "Henüz bir veri seti yüklenmedi!")
            return

        secilen_yontem = self.gurultuluform.comboBox_EksikVerilerDoldur.currentText().strip()

        try:
            if secilen_yontem == "En çok tekrar eden değer ile":
                self.veri_seti = self.veri_seti.apply(
                    lambda col: col.fillna(col.mode()[0]) if col.isnull().any() else col)
            elif secilen_yontem == "Ortalamayla":
                self.veri_seti = self.veri_seti.apply(lambda col: col.fillna(col.mean()) if col.isnull().any() else col)
            elif secilen_yontem == "Ortanca ile":
                self.veri_seti = self.veri_seti.apply(
                    lambda col: col.fillna(col.median()) if col.isnull().any() else col)
            elif secilen_yontem == "Sıfır(0) ile":
                self.veri_seti = self.veri_seti.fillna(0)
            else:
                QMessageBox.warning(self, "Uyarı", "Geçersiz doldurma yöntemi seçildi.")
                return

            QMessageBox.information(self, "Başarılı", f"Veriler {secilen_yontem} ile başarıyla dolduruldu!")

        except Exception as e:
            QMessageBox.critical(self, "Hata", f"Veri doldurulurken bir hata oluştu:\n{str(e)}")
